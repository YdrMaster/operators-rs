...
use cuda::{AsRaw, DevByte, Stream, bindings::MCstream};
...
macro_rules! extern_c {
    ($ty:ty; $lib:expr, $name:expr; $($args:expr),* $(,)?) => {{
        ...
        if result == ::cuda::bindings::mcError_t::mcSuccess as _ {
            Ok(())
        } else {
...
{EXPORT}mcError_t {workspace_name}(
    size_t *argmax,
